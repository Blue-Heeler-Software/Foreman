#!/usr/bin/env bash
# Remove the Ubuntu Foreman binaries and launchers while preserving evidence.
...
cat <<'EOF'
Foreman Agent Safety was removed.

Configuration and audit evidence were deliberately preserved in:
  ~/.config/foreman
  ~/.local/state/foreman
  ~/.local/share/foreman
